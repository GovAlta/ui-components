...
previewUrl: https://govalta.github.io/goa-workspace-playground/cases
...
The index page is built around a <a href={withBase("/examples/workspace#data-views")}>data view</a> showing the records the user is responsible for. Tabs filter by status. Search and filter chips narrow further. A primary action button (often "New record") lives in the page header.
...
- Pair with <a href={withBase("/examples/workspace#sidebar-navigation")}>sidebar navigation</a> so staff can jump between index pages for different record types.
