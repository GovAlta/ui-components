...
<div style="display:flex;gap:1rem;flex-wrap:wrap;align-items:center;margin-top:0.5rem;margin-bottom:2rem;">
  <goa-button version="2" type="secondary" onclick="window.open('https://govalta.github.io/goa-workspace-playground/', '_blank')">View demo</goa-button>
...
<goa-callout version="2" type="information" emphasis="low" heading="Workspace layout component" maxwidth="80ch">
  ...
  <div style="margin-top: var(--goa-space-s, 0.75rem);">
    <goa-link size="small"><a href="/components/workspace-layout">View layout component</a></goa-link>
  </div>
</goa-callout>
...
Use the [work side menu](/components/work-side-menu) as the primary navigation for a workspace. The [workspace layout](/components/workspace-layout) provides this navigation, including the collapse to a hamburger on mobile.
...
A scroll-aware header that pins to the top of the content area as the user scrolls. Use it to keep the most important tools visible. The [workspace layout](/components/workspace-layout) provides this as its sticky page header.
...
Use a [push drawer](/components/push-drawer) for focused work that should happen without leaving the current page. The [workspace layout](/components/workspace-layout) hosts this as a push drawer that narrows the main card rather than overlaying it.
