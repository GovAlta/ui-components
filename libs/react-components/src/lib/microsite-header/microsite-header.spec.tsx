...
import ABGovMicrositeHeader, { ServiceLevel } from "./microsite-header";

describe("Header", () => {
  (["alpha", "beta", "live"] as const).forEach((mode: ServiceLevel) => {
    it("should show the all the flairs", async function() {
      render(<ABGovMicrositeHeader type={mode} />);
  ...
  it("should show the all the flairs", async function() {
    render(<ABGovMicrositeHeader type="alpha" version="v1.2.3" />);
    const el = document.querySelector("goa-microsite-header");
    expect(el?.getAttribute("version")).toEqual("v1.2.3");
  });
  it("should set self url target", () => {
    render(<ABGovMicrositeHeader type="alpha" headerUrlTarget="self" feedbackUrlTarget="self" />);
    const el = document.querySelector("goa-microsite-header");
