...
describe("Temporary Notification", () => {
  ...
  it("should render with visibility", async () => {
    ...
    const el = document.querySelector("goa-temp-notification");
    expect(el?.getAttribute("visible")).toEqual("false");
  });
  ...
  it("Event triggered on action", async () => {
    const onAction = vi.fn();
    const { container } = render(
      <GoabTemporaryNotification message="Test message" actionText="Dismiss" onAction={onAction} />
    );
    const notification = container.querySelector("goa-temp-notification");
    notification && fireEvent(notification, new CustomEvent("action"));
