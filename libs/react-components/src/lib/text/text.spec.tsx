...
describe("GoabText", () => {
  ...
  it("should handle tag property with other properties", () => {
    const { container } = render(
      <GoabText tag="p" size="body-l" maxWidth="480px" color="primary" mt="m" mb="l">
        Paragraph Text
      </GoabText>,
    );
  ...
  it("should handle different tag values", () => {
    ...
    cases.forEach(({ tag, name }) => {
      const { container } = render(
        <GoabText tag={tag as GoabTextTextElement | GoabTextHeadingElement}>
          {name}
        </GoabText>,
      );
  ...
  it("should pass data-grid attributes", () => {
    const { container } = render(<GoabText data-grid="cell">Test content</GoabText>);
