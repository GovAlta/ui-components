...
import { GoabxFilterChip } from "./filter-chip";
import { describe, it, expect, vi } from "vitest";

describe("GoabxFilterChip", () => {
  it("should render", () => {
    const { container } = render(<GoabxFilterChip content="some filter chip" />);
  ...
  it("should show the chip in the error state", () => {
    const { container } = render(<GoabxFilterChip content="Some Badge" error={true} />);
    const el = container.querySelector("goa-filter-chip");
    expect(el?.getAttribute("error")).toBe("true");
  });
  ...
  it("should handle the click event", async () => {
    const onClick = vi.fn();
    const { container } = render(
      <GoabxFilterChip content="Some Badge" onClick={onClick} testId="chip" />,
    );
    const el = container.querySelector("goa-filter-chip");
  ...
  it("should have an unfilled close icon by default", () => {
    const { container } = render(<GoabxFilterChip content="Test" />);
    const el = container.querySelector("goa-filter-chip");
  ...
  it.skip("should not apply background fill on hover", async () => {
    const { container } = render(<GoabxFilterChip content="Test" testId="chip" />);
  ...
  it("should pass data-grid attributes", () => {
    const { container } = render(
      <GoabxFilterChip content="test chip" data-grid="cell" />,
    );
    const el = container.querySelector("goa-filter-chip");
