...
describe("Calendar", () => {
  ...
  it("dispatches a change event when a date is clicked", async () => {
    const handleChange = vi.fn();
    ...
    await vi.waitFor(() => {
      ...
      expect(handleChange).toHaveBeenCalledWith({
        name: "",
        value: expect.stringContaining(todayStr),
      });
    });
  });

  it("dispatches change event with name when provided", async () => {
    const handleChange = vi.fn();
    const today = new Date();
    const todayStr = format(today, "yyyy-MM-dd");
  ...
  it("respects min date constraint", async () => {
    ...
    await vi.waitFor(() => {
      expect(pastDateButton.element().classList.contains("disabled")).toBe(
        true
      );
    });
  ...
  it("respects max date constraint", async () => {
    ...
    await vi.waitFor(() => {
      expect(futureDateButton.element().classList.contains("disabled")).toBe(
        true
      );
    });
  ...
  describe("Month and Year selection", () => {
    it("changes the month when month dropdown is changed", async () => {
      ...
      container.element().addEventListener("_change", (e: Event) => {
      ...
      await vi.waitFor(() => {
        expect(monthsDropdown.element()).toBeTruthy();
      });
    });
    ...
    it("changes the year when year dropdown is changed", async () => {
      ...
      await vi.waitFor(() => {
        expect(yearsDropdown.element()).toBeTruthy();
      });
    });
  });
  ...
  describe("Keyboard Navigation", () => {
    it("navigates to previous day with ArrowLeft and selects with Enter", async () => {
      ...
      const result = render(<Component />);
      const startButton = result.getByTestId(startDateStr);
  ...
  describe("Visual states", () => {
    ...
    it("highlights the selected date", async () => {
      ...
      await vi.waitFor(() => {
        expect(selectedButton.element().classList.contains("selected")).toBe(
          true
        );
      });
    });
    ...
    it("displays days from previous and next months", async () => {
      ...
      await vi.waitFor(() => {
        ...
        expect(lastDayOfFeb.element().classList.contains("other-month")).toBe(
          true
        );
      });
    });
  ...
  describe("Edge cases", () => {
    it("handles leap year correctly", async () => {
      ...
      await vi.waitFor(() => {
        ...
        expect(leapDayButton.element().classList.contains("selected")).toBe(
          true
        );
      });
    });
