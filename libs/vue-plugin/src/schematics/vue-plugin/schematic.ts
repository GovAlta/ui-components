...
interface NormalizedSchema extends VuePluginSchematicSchema {
  projectName: string;
  projectRoot: string;
...
function normalizeOptions(options: VuePluginSchematicSchema): NormalizedSchema {
...
export default function (options: VuePluginSchematicSchema): Rule {
  ...
  return chain([
    updateWorkspace((workspace) => {
      ...
      targets.add({
        name: 'build',
        builder: './dist/libs/vue-plugin:lib',
        options: {
