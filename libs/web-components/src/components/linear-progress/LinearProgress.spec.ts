...
describe("GoALinearProgress", () => {
  [-1, 0, 20, 100, 101].forEach((progress: number) => {
    it(`renders the linear progress with progress value ${progress}%`, async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          container
            .querySelector(".progressbar-container")
  ...
  it("renders indeterminate progress when progress is not provided", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        container.querySelector(".progressbar-indeterminate-indicator"),
      ).not.toBeNull();
      expect(
        container.querySelector(".progressbar-determinate-indicator"),
      ).toBeNull();
    });
  });
  ...
  it("does render aria-valuetext attribute when progress is indeterminate", async () => {
    const { container } = render(GoALinearProgress, { progress: 100 });
