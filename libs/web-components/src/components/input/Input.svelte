...
<script lang="ts">
  ...
  function setValue(detail: FieldsetSetValueRelayDetail) {
    // @ts-expect-error
    value = detail.value;
    const syntheticEvent = new Event("goa-input:set-value");
    dispatchOnChange(value, syntheticEvent);
  }
    ...
    _debounceId = setTimeout(() => {
      input.dispatchEvent(
        new CustomEvent("_change", {
          ...
          bubbles: true,
          cancelable: true,
          detail: { name, value: input.value, event: originalEvent },
        }),
      );
    }, debounce);
  ...
  function doClick(this: HTMLElement) {
    this.dispatchEvent(
