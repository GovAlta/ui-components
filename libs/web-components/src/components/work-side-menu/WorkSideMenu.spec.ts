...
describe("WorkSideMenu", () => {
  ...
  it("renders", async () => {
    const { container } = render(WorkSideMenuWrapper, menuOptions);
  ...
  describe("Mobile", () => {
    ...
    it("toggles menu when firing a toggle event", async () => {
      ...
      await waitFor(() => expect(menu.classList.contains("closed")).toBeTruthy(), {timeout: 1} );

      // open
      menu.dispatchEvent(new CustomEvent("work-side-menu:toggle"));
      await waitFor(() => expect(menu.classList.contains("closed")).toBeFalsy(), {timeout: 1} );
    });
  });
