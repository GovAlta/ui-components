...
<script lang="ts">
  ...
  const _iconOverrides: Record<GoAIconOverridesType & GoAIconOverridesTypeWithTheme, string> = {
  ...
  onMount(async () => {
    if (ariacontrols != "" || ariaexpanded != "") {
      console.warn(
        "goa-icon [ariacontrols] and [ariaexpanded] properties are deprecated.",
      );
    }
  });
