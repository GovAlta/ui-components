    ...
    function findAllFocusableNodes(nodes: NodeList | Node[]): void {
      for (const node of nodes) {
        ...
        if (focusableNode && node !== cell) {
          ...
          // some elements inside component like FocusTrap, even set tabindex >=0, but shouldn't be added to this array (ex: dropdown has popover has 2 spans inside it, shouldn't be added)
          if (element.hasAttribute && element.hasAttribute('data-ignore-focus-datagrid')) {
            continue;
          }
