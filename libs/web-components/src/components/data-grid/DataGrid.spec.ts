import { render, fireEvent, createEvent, waitFor } from "@testing-library/svelte";
...
// Helper to wait for the DataGrid's setTimeout delay for shadow DOM rendering
const waitForGridSetup = async () => {
  await tick();
  await new Promise(resolve => setTimeout(resolve, 150)); // Wait for 100ms setTimeout + buffer
  await tick();
};
...
describe("GoADataGrid", () => {
  ...
  describe("Keyboard Icon Property", () => {
    it("should not show keyboard icon when keyboardIconVisibility is hidden", async () => {
      const { container } = render(DataGrid, { props: { keyboardIconVisibility: "hidden" } });
      await waitForGridSetup();
      ...
      // Keyboard icon should not be rendered when visibility is hidden
      const keyboardIndicator = container.querySelector('.keyboard-indicator.show');
      expect(keyboardIndicator).toBeFalsy();
    });

    it("should show keyboard icon when keyboardIconVisibility is visible", async () => {
      const { container } = render(DataGrid, { props: { keyboardIconVisibility: "visible" } });
      await waitForGridSetup();
      ...
      // based on keyboardIconVisibility prop
    });
  });
