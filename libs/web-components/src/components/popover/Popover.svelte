...
<script lang="ts">
  ...
  onMount(async () => {
    ...
    document.body.addEventListener("goa:closePopover", (e: Event) => {
      // This commented out code was due to the goa:closePopover message closing the popover that is trying to be opened
      // Not
  ...
  function getFirstFocusableEl(el: HTMLElement): HTMLElement | null {
    ...
    const children = [...el.children, ...getSlottedChildren(el), el.shadowRoot].filter(Boolean) as HTMLElement[];
