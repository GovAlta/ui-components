...
export const MaxWidthTemplate = (args) => {
  const [showModal, setShowModal] = useState(false);
  const [maxWidth, setMaxWidth] = useState("90vw");
  return (
    <>
      <GoAFormItem label="Max Width" helpText="Enter width with unit (px,vw,rem,em,ch)">
        <GoAInput onChange={(name, val) => setMaxWidth(val)} value={maxWidth} />
      </GoAFormItem>
      <GoAButton mt="l" onClick={() => setShowModal(true)}>Open Modal</GoAButton>
      <GoAModal
        heading="Do you agree?"
        maxWidth={maxWidth}
        open={showModal}
...
<Story name="Max Width">{MaxWidthTemplate.bind({})}</Story>
...
<Tabs>
  <Tab label="Angular">
    <CodeSnippet
      lang="html"
      code={`
        <goa-button (_click)="openModal()">Open Modal</goa-button>
        <goa-modal 
          [open]="isOpen" 
          (_close)="closeModal()" 
          closable 
