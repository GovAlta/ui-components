...
export const BasicTemplate = (args) => {
  ...
  return (
    <GoARadioGroup name={args.name} orientation={args.orientation || "vertical"} value={value} onChange={onChange}>
...
<Story name="Basic" args={{name: "basic"}}>{BasicTemplate.bind({})}</Story>
...
<Story name="Orientation" args={{name: "orientation", orientation: "horizontal"}}>
  {BasicTemplate.bind()}
</Story>
