...
<Props showTabs={true}>
  ...
  <Prop
    ...
    lang="angular"
    type="string"
    description="Defines how the input will be translated for the screen reader. If not specified it will fall back to the name. If both arialabel and arialabelledby are specified, arialabelledby will be used"
  />
  <Prop
    name="ariaLabel"
    lang="react"
...
<Tabs>
  ...
  <Tab label="Angular">
    <CodeSnippet
      code={`
        <goa-input disabled="true" placeholder="Find by name"></goa-input>
        <!-- Use [attr.disabled] with [formControl] -->
        <goa-input goaValue [formControl]="reactiveFormCtrl" [attr.disabled]="true" placeholder="Find by name"></goa-input>
      `}
      lang="html"
    />
    <CodeSnippet
...
export const MaxLengthTemplate = () => {
  const [value, setValue] = useState("");
  return (
    <GoAFormItem label="Name" helpText={"Max length is 10"}>
      <GoAInput
        maxLength={10}
        name="name"
        onChange={(name, value) => setValue(value)}
...
<Story name="Max Length">{MaxLengthTemplate.bind({})}</Story>

<Tabs>
  <Tab label="Angular">
    <CodeSnippet
      code={`<goa-form-item label="Name" helptext="Max length is 10.">
<goa-input name="name" maxLength="10"></goa-input>
</goa-form-item>`}
      lang="html"
    />
  </Tab>
  <Tab label="React">
    <CodeSnippet
      ...
      return (
        <GoAFormItem label="Name" helpText={"Max length is 10"}>
            <GoAInput maxLength={10} name="name" onChange={(name, value) => setValue(value)} value={value}/>
        </GoAFormItem>
      )
    `}
      lang="tsx"
...
When working with `type=time` the `step` property controls the seconds step value (set to 60 if you don't want seconds)
