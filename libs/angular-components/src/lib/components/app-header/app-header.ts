...
import { GoabSlottedTemplate } from "../../slotted-directive";

@Component({
  standalone: true,
  selector: "goab-app-header",
  imports: [NgTemplateOutlet, GoabSlottedTemplate],
  template: `@if (isReady) {
    <goa-app-header
      [attr.url]="url"
    ...
    >
      ...
      @if (navigation) {
        <ng-container
          [goabSlottedTemplate]="navigation"
          slotName="navigation"
        ></ng-container>
      }
      @if (utilities) {
        <ng-container
          [goabSlottedTemplate]="utilities"
          slotName="utilities"
        ></ng-container>
      }
      <ng-content />
