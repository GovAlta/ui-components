...
@Component({
  ...
  template: `
    ...
    >
      ...
      <ng-template #pushDrawer>
        <div class="test-push-drawer">Push Drawer Content</div>
      </ng-template>
...
describe("GoabWorkspaceLayout", () => {
  ...
  it("should render push-drawer slot content", () => {
    const el = fixture.debugElement.query(By.css("goa-workspace-layout"))?.nativeElement;
    const slot = el?.querySelector("[slot='push-drawer']");
    expect(slot).toBeTruthy();
    expect(slot?.innerHTML).toContain("Push Drawer Content");
  });
