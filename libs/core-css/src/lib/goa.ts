...
export const goa = new (function () {
  ...
  this.ui = new (function () {
    ...
    this.accordion = function () {
      ...
      for (var c = 0; c < divComponent.length; c++) {
        ...
        if (liItems.length > 1) {
          ...
          aExpandAll.appendChild(
            document.createTextNode(o.labels[o.data.language].accordion.expand)
          );
          ...
          aExpandAll.onclick = (function (divComponent, liItems) {
            return function () {
              for (var c = 0; c < liItems.length; c++) {
                ...
                divTitle.children[0].setAttribute('aria-expanded', 'true');
                divText.className = divText.className.replace(
                  'collapsed',
                  'expanded'
                );
              }
          ...
          aCollapseAll.appendChild(
            document.createTextNode(
              o.labels[o.data.language].accordion.collapse
            )
          );
          ...
          aCollapseAll.onclick = (function (divComponent, liItems) {
            return function () {
              for (var c = 0; c < liItems.length; c++) {
                ...
                divTitle.children[0].setAttribute('aria-expanded', 'false');
                divText.className = divText.className.replace(
                  'expanded',
                  'collapsed'
                );
        ...
        for (let c2 = 0; c2 < liItems.length; c2++) {
    ...
    this.locations = function () {
      ...
      for (var c = 0; c < a.length; c++) {
        ...
        for (var d = 0; d < l.length; d++) {
          p[d] = {
            m: new google.maps.Marker({
              ...
              position: {
                lat: parseFloat(
                  (<HTMLElement>l[d].getElementsByClassName('latitude')[0])
                    .innerText
                ), // 53.536673247532704,
                lng: parseFloat(
                  (<HTMLElement>l[d].getElementsByClassName('longitude')[0])
                    .innerText
                ), // -113.50505621957399
              },
          ...
          if (
            localities.indexOf(
              (<HTMLElement>(
                l[d].querySelectorAll('[itemprop="addressLocality"]')[0]
              )).innerText
            ) == -1
          ) {
            localities.push(
          ...
          b.appendChild(
            document.createTextNode(
              o.labels[o.data.language].locations.showOnMap
            )
          );
          l[d].appendChild(b);
        }
        ...
        divFilterText.innerHTML =
          '<strong>' +
          l.length +
          '</strong> location' +
          (l.length > 1 ? 's' : '') +
          ' in <strong>all cities</strong> found:';
        ...
        formLocaleFilter.onsubmit = (function (
          selectLocaleFilter,
          divFilterText,
          p,
          l
          ...
          return function () {
            ...
            for (var c = 0; c < p.length; c++) {
              if (selectLocaleFilter.value == 'All') {
                ...
                if (
                  selectLocaleFilter.value ==
                  (<HTMLElement>(
                    l[c].querySelectorAll("[itemprop='addressLocality']")[0]
                  )).innerText
                ) {
        ...
        labelLocaleFilter.appendChild(
          document.createTextNode(
            o.labels[o.data.language].locations.searchLabel
          )
        );
        buttonLocaleFilter.appendChild(
          document.createTextNode(
            o.labels[o.data.language].locations.searchButton
          )
        );

        selectLocaleFilter.required = true;
    ...
    this.tables = function () {
      ...
      for (var c = 0; c < divComponents.length; c++) {
        ...
        if (divComponent.scrollWidth == divComponent.clientWidth) {
          ...
          divComponent.scrollLeft ==
          divComponent.scrollWidth - divComponent.clientWidth
        ) {
        ...
        divTableOuter.firstChild.addEventListener(
          'scroll',
          (function (divComponent, divTableOuter) {
            return function () {
              if (divComponent.scrollWidth == divComponent.clientWidth) {
                divTableOuter.className = 'goa-tableouter';
              } else if (divComponent.scrollLeft == 0) {
  ...
  this.init = function (params) {
    ...
    if (o.data.locations.length > 0) {
      ...
      s.src =
        'https://maps.googleapis.com/maps/api/js?key=AIzaSyDgDWjTlfR1LjJ74kvBfx8VDZ_ZamSos4M&callback=goa.ui.locations&language=' +
        o.data.language.slice(0, 2) +
        '&region=' +
        o.data.language.slice(3, 5);
    ...
    if (o.data.forms.length > 0) {
      var s = document.createElement('script');
      s.src =
        'https://www.google.com/recaptcha/api.js?onload=initAlbertaForms&render=explicit&hl=' +
        o.data.language;
      document.body.appendChild(s);
    }
  ...
  (function () {
    ...
    var params =
      location.href.indexOf('?') > -1
        ? location.href.slice(location.href.indexOf('?') + 1).split('&')
        : new Array();
    var nameValue = [];
    var c = 0;
