...
function makeServer(dataLoader: DataLoader, version: string): McpServer {
...
function resolveTransport(): Transport {
  ...
  process.stderr.write(
    `[design-system-mcp] Unknown MCP_TRANSPORT="${raw}". Valid values: stdio, http. Defaulting to stdio.\n`,
  );
  return 'stdio';
}
...
async function main() {
  ...
  process.stderr.write(`[design-system-mcp] transport=${transport}\n`);

  if (transport === 'http') {
    await startHttp(() => makeServer(dataLoader, packageVersion), itemCount, packageVersion);
  } else {
    const server = makeServer(dataLoader, packageVersion);
    await server.connect(new StdioServerTransport());
    process.stderr.write(`GoA Design System MCP v${packageVersion} ready (${itemCount} items loaded)\n`);
  }
}
...
async function startHttp(
  ...
  app.get('/health', (_req, res) => {
    res.json({ status: 'ok', itemsLoaded: itemCount });
  });
  ...
  return new Promise((resolve) => {
    const httpServer = app.listen(port, () => {
      process.stderr.write(`GoA Design System MCP v${version} ready (${itemCount} items loaded)\n`);
      resolve();
    });
