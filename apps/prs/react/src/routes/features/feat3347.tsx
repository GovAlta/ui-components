 ...
 *
 * Push Drawer V2 lives at features/3347-push (its own full-page route).
 */
...
export function Feat3347Route() {
  ...
  return (
    <>
      ...
      <p>
        Fixed height 480px. Header + scrollable body + footer. This is where the
        component shines.
      </p>
      <GoabScrollPanel
        height="480px"
      ...
      >
        <div style={demoBodyStyle}>
          {paragraphs.map((n) => (
            <p key={n}>
              Paragraph {n} — Lorem ipsum dolor sit amet, consectetur adipiscing
      ...
      <p>
        Verifies Drawer V2 still works after its internal scroll mechanics were
        replaced with <code>&lt;goa-scroll-panel&gt;</code>. Open and scroll
        inside — header should stay pinned at top (with border on scroll),
        actions footer should stay pinned at bottom (with border when content
        overflows above).
      </p>
      ...
      >
        {paragraphs.map((n) => (
          <p key={n}>
            Paragraph {n} — Drawer V2 now uses goa-scroll-panel internally.
            Verify header sticky + actions sticky + content scrolls + borders
            appear when scrolled (top/middle/bottom states).
          </p>
        ))}
      </GoabDrawer>
      ...
      <p>
        Push Drawer V2 also got the scroll-panel treatment — see the dedicated
        full-page test at <code>features/3347-push</code> so the drawer's
        internal scroll can be verified without page scroll bubbling.
      </p>
      ...
      <p>
        ...
        goa-scrollable + maxheight calc). The <code>GoabModal</code> React/
        Angular wrapper hardcodes <code>version="2"</code> so this test
        exercises the new path. Open and scroll inside — header should stay
        pinned at top with border on scroll, action footer pinned at bottom with
        border when content overflows above.
      </p>
      ...
      <p>
        Verifies the notification panel still works after its layout was
        refactored to use <code>&lt;goa-scroll-panel&gt;</code> internally
        (header slot for title + tabs, default slot for the scrollable
        notification list, footer slot for "View all" / "Mark all as read"
        actions). Rendered inline here (normally lives inside a popover
        triggered from a work-side-menu icon). Scroll inside the notification
        list to verify sticky header + sticky footer + border feedback on
        scroll.
      </p>
