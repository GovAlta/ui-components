...
Then(/^cs I should be able to validate dropdown button css property$/, function () {
  ...
  cy.get('goa-input[id="colors-dropdown-input"]').find('div.goa-input.variant--goa.type--text').should("have.css", "border", "1px solid " + hexToRgb(properties["color-gray-600"]));
...
Then(/^cs I should be able to validate dropdown menu css property$/, function () {
  ...
  cy.get('goa-input[id="colors-dropdown-input"]').get('ul[data-testid="dropdown-menu"]').find('li[data-index="0"]').eq(0).trigger('mouseover').should("have.css", "background", hexToRgb(properties["color-gray-100"]) + " none repeat scroll 0% 0% / auto padding-box border-box");
  cy.get('goa-input[id="colors-dropdown-input"]').get('ul[data-testid="dropdown-menu"]').find('li[data-index="1"]').eq(1).click({ force: true, multiple: true }).should("have.css", "background", hexToRgb(properties["goa-color-interactivehover"]) + " none repeat scroll 0% 0% / auto padding-box border-box");
});
