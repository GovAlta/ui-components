...
Then(/^cs I should be able to validate dropdown error button css property$/, function () {
  ...
  cy.get('goa-input[id="colors2-dropdown-input"]').find('div.goa-input.variant--goa.type--text.error').should("have.css", "border", "2px solid " + hexToRgb(properties["goa-color-interactiveerror"]));
...
Then(/^cs I should be able to validate dropdown error menu css property$/, function () {
  ...
  cy.get('goa-input[id="colors2-dropdown-input"]').get('ul[data-testid="dropdown-menu"]').find('li[data-index="1"]').eq(1).click({ force: true, multiple: true }).should("have.css", "background", hexToRgb(properties["goa-color-interactivehover"]) + " none repeat scroll 0% 0% / auto padding-box border-box");
});
