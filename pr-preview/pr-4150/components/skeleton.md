...
| `type` | GoabSkeletonType | — | Yes | Sets the skeleton shape to represent your content. |
...
| Prop | Type | Default | Required | Description |
|------|------|---------|----------|-------------|
| `lineCount` | number | — | No | Used within components that contain multiple lines. Currently only used in card skeleton type. |
| `maxWidth` | string | `300px` | No | Sets the maximum width. Currently only used in card skeleton type. |
...
| `size` | GoabSkeletonSize | — | No | Size can affect either the height, width or both for different skeleton types. |
...
| `testid` | string | — | No | Sets a data-testid attribute for automated testing. |
| `type` | "image" | "text" | "title" | "text-small" | "avatar" | "header" | "paragraph" |… | — | Yes | Sets the skeleton shape to represent your content. |
...
- ⚠️ **Warning:** Show an error state if loading takes too long. Don't leave skeletons showing indefinitely without feedback.
