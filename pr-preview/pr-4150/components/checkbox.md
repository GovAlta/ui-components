...
| `checked` | boolean | — | No | Marks the checkbox item as selected. |
| `description` | string | React.ReactNode | — | No | Additional description text displayed below the checkbox label. |
| `disabled` | boolean | — | No | Disable this control. It will not receive focus or events. |
| `error` | boolean | — | No | Shows an error on the checkbox item. |
| `id` | string | — | No | Sets a unique id for the checkbox element. |
| `indeterminate` | boolean | — | No | Shows a mixed/partial selection state. Used for 'Select All' checkboxes when some items are selected. |
| `maxWidth` | string | — | No | Sets the maximum width of the checkbox. |
| `mb` | Spacing | — | No |  |
| `ml` | Spacing | — | No |  |
| `mr` | Spacing | — | No |  |
| `mt` | Spacing | — | No |  |
| `name` | string | — | Yes | Unique name to identify the checkbox. |
...
| `text` | string | — | No | Label shown beside the checkbox. |
| `value` | string | number | boolean | — | No | The value binding. |
...
| `ariaLabel` | string | — | No | Defines how the text will be translated for the screen reader. If not specified it will fall back to the name. |
| `checked` | boolean | — | No | Marks the checkbox item as selected. |
| `description` | string | TemplateRef<any> | — | No | Sets additional description content displayed below the checkbox label. Accepts plain text or a template. |
...
| `name` | string | — | No | Sets the name of the checkbox input for form submission. |
| `revealArialLabel` | string | — | No | Text announced by screen readers when the reveal slot content is displayed. |
| `size` | GoabCheckboxSize | `default` | No | Sets the size of the checkbox. 'compact' reduces spacing for dense layouts. |
| `testId` | string | — | No | Sets the data-testid attribute for automated testing. |
| `text` | string | — | No | Label shown beside the checkbox. |
| `value` | string | number | boolean | null | — | No | The value binding. |
...
| `description` | string | — | No | Additional description text displayed below the checkbox label. |
...
| `testid` | string | — | No | Sets a data-testid attribute for automated testing. |
| `text` | string | — | No | Label shown beside the checkbox. |
| `value` | string | — | No | The value binding. |
...
| Event | Type | Description |
|-------|------|-------------|
| `_blur` | CustomEvent<{ name: string; value: string; checked: boolean }> |  |
| `_change` | CustomEvent<{ name: string; checked: boolean; value: string }> |  |
| `_focus` | CustomEvent<{ name: string; value: string; checked: boolean }> |  |
