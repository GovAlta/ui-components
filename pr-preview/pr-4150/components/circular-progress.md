...
|------|------|---------|----------|-------------|
| `message` | string | — | No | Loading message displayed below the progress indicator. |
| `progress` | number | — | No | Sets the progress value (0–100). When omitted, an infinite spinner is shown. |
...
| `visible` | boolean | — | No | Controls visibility of the progress indicator, allowing a fade transition to be applied. |
...
| Prop | Type | Default | Required | Description |
|------|------|---------|----------|-------------|
| `message` | string | — | No | Loading message displayed under the progress indicator. |
| `progress` | number | — | No | Set the progress value. Setting this value will change the type from infinite to progress. |
| `size` | GoabCircularProgressSize | — | No | Size of the progress indicator. |
| `testId` | string | — | No | Sets a data-testid attribute for automated testing. |
| `variant` | GoabCircularProgressVariant | — | No | Stretch across the full screen or use it inline. |
| `visible` | boolean | — | No | Show/hide the page loader. This allows for fade transition to be applied in each transition. |
...
- ⚠️ **Warning:** CircularProgress is hidden by default. Always set visible={true} to show it.
