import{j as t}from"./jsx-runtime.D_zvdyIk.js";import{r as s}from"./index.DeO6U63H.js";import{G as y}from"./tooltip.B2v1476D.js";import"./workspace-layout-scroll-state.LWqBXkyB.js";import{CodeSnippet as k}from"./CodeSnippet.Cs0Eii2O.js";import{p as S}from"./purify.es.D6gMnemd.js";import"./extract-props.mAATCLkF.js";import"./button.Cr0WWE4a.js";import"./framework-preference.DlzkYYed.js";/* empty css                             */const N=5*60*1e3;function E(r){const[u,a]=s.useState(null);return s.useEffect(()=>{if(!r)return;const d=`gh-issues-${r}`;try{const n=sessionStorage.getItem(d);if(n){const{count:o,timestamp:l}=JSON.parse(n);if(Date.now()-l<N){a(o);return}}}catch{}(async()=>{try{const n=encodeURIComponent(`repo:GovAlta/ui-components is:issue is:open label:"${r}"`),o=await fetch(`https://api.github.com/search/issues?q=${n}&per_page=1`,{headers:{Accept:"application/vnd.github.v3+json"}});if(!o.ok){console.warn("GitHub API error:",o.status),a(0);return}const p=(await o.json()).total_count||0;a(p);try{sessionStorage.setItem(d,JSON.stringify({count:p,timestamp:Date.now()}))}catch{}}catch(n){console.warn("Failed to fetch GitHub issue count:",n),a(0)}})()},[r]),u}const I={CUSTOM_ELEMENT_HANDLING:{tagNameCheck:/^goa-/,attributeNameCheck:()=>!0}};function M({configurations:r,figmaUrl:u,githubUrl:a,componentName:d}){const[g,n]=s.useState(r.defaultConfigurationId),o=s.useRef(null),l=s.useRef(null),p=E(d),i=r.configurations.find(e=>e.id===g);s.useEffect(()=>{if(o.current&&i){const e=i.code.webComponents,c=typeof e=="string"?e:[e.css?`<style>${e.css}</style>`:"",e.html,e.js?`<script>${e.js}<\/script>`:""].join(""),m=r.previewWrapper?r.previewWrapper.replace("{{slot}}",c):c,x=m.match(/<script>([\s\S]*?)<\/script>/i),w=x?x[1]:null,b=S.sanitize(m,I);if(o.current.innerHTML=b.replace(/<goa-(?!microsite-header)([a-z-]+)(?![^>]*version=)/g,'<goa-$1 version="2"').trim(),w)try{const f=o.current,C=w.replace(/document\.getElementById\s*\(\s*["']([^"']+)["']\s*\)/g,(j,v)=>`container.querySelector("#${v}")`).replace(/document\.querySelector\s*\(\s*["']([^"']+)["']\s*\)/g,(j,v)=>`container.querySelector("${v}")`);new Function("container",C)(f)}catch(f){console.error("Error executing configuration script:",f)}}},[i]);const h=s.useCallback(e=>{const c=Array.isArray(e.value)?e.value[0]:e.value;n(c)},[]);return s.useEffect(()=>{const e=l.current;if(!e)return;const c=m=>{h(m.detail)};return e.addEventListener("_change",c),()=>e.removeEventListener("_change",c)},[h]),s.useLayoutEffect(()=>{o.current&&r.previewStyle&&(o.current.style.cssText=r.previewStyle)},[r.previewStyle]),i?t.jsxs("div",{className:"configuration-preview",children:[t.jsxs("div",{className:"control-bar",children:[t.jsx("div",{className:"config-dropdown",children:t.jsx("goa-dropdown",{name:"configuration",value:g,version:"2",size:"compact",maxheight:"600px",ref:l,children:r.configurations.map(e=>t.jsx("goa-dropdown-item",{value:e.id,label:e.name},e.id))})}),t.jsxs("div",{className:"external-links",children:[u&&t.jsx(y,{content:"View Figma component",children:t.jsx("a",{href:u,target:"_blank",rel:"noopener noreferrer",className:"external-link","aria-label":"View Figma component",children:t.jsx("goa-icon",{type:"logo-figma",size:"medium"})})}),a&&t.jsx(y,{content:"View GitHub issues",children:t.jsxs("a",{href:a,target:"_blank",rel:"noopener noreferrer",className:"external-link github-link","aria-label":"View GitHub issues",children:[t.jsx("goa-icon",{type:"logo-github",size:"medium"}),p!==null&&t.jsxs("span",{className:"issue-count",children:["(",p,")"]})]})})]})]}),t.jsx("div",{className:"preview-area",children:t.jsx("div",{className:"preview-container",ref:o})}),t.jsx("div",{className:"code-area",children:t.jsx(k,{frameworkCode:{react:i.code.react,angular:i.code.angular,webComponents:i.code.webComponents},maxHeight:200,showCopy:!0})}),t.jsx("style",{children:`
      ...
      `})]}):t.jsx("div",{children:"No configuration found"})}export{M as default};
